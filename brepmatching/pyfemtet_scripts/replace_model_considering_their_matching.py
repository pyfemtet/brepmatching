...
def update_model_with_prediction(
    ...
    topo_id_vs_exp_id_var = get_topo_id_vs_export_id(new_xt_path, Femtet.hWnd)
...
if __name__ == '__main__':
    ...
    def update_model():
        if not Femtet.UpdateVariable('r', 4):
            Femtet.ShowLastError()
        if not Femtet.UpdateVariable('r2', 4):
            Femtet.ShowLastError()
        if not Femtet.UpdateVariable('r3', 4):
            Femtet.ShowLastError()
        Femtet.Gaudi.LastXTPath = r'C:\temp\model_var.x_t'
        Femtet.Gaudi.ReExecute()
        Femtet.Redraw()

    update_model_with_prediction(
        Femtet=Femtet,
